...
class Notify extends \WxPayNotify
{
    ...
    public function processOrder($order, $data)
    {
        ...
        //如果有使用优惠券，更新用户优惠券状态
        if ($order['uc_id'] > 0){
            db('user_coupon')->where('id', $order['uc_id'])->update(['is_use'=>1,'usage_time'=>$time,'o_no'=>$order['o_no']]);
        }

        //当订单实付金额大于等于10元时，推送一张编号为2的优惠券
        /*if($order['total']>=10){
            $arrParam = array(
                'uid' => $order['uid'],
                'coupon_id' => 2,
            ...
            db('user_coupon')->insert($arrParam);
        }*/

        $order_detail_list = db('order_detail')->field('name,barcode,price,goodid,num')->where('oid', $order['id'])->select();

        if(!empty($order_detail_list)){

            foreach ($order_detail_list as $key=>$item){
               db('shelves_product')->where(array('shelves_id'=>$order['sid'],'product_id'=>$item['goodid']))->setDec('num',$item['num']);
                ...
                $param[$key] = array(
                    ...
                    'p_price' => $item['price'],
                    'p_num' => $item['num'],
                    'p_total' => $item['price']*$item['num'],
                    'total' => $order['total'],
                    'all_total' => $order['all_total'],
                    'uc_id' => $order['uc_id'],
    ...
    public function getOrder($data)
    {
        // 可根据商户订单号进行查询
        // 例如:
        $order = db('order')->where('o_no', $data['out_trade_no'])->find();
        return $order;
    }
