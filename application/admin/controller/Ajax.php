...
class Ajax extends Backend
{
    ...
    public function upload_oss(){
        ...
        if ($splInfo)
        {
            ...
            $key_id = config('ALIOSS_CONFIG.KEY_ID');
            $key_secret = config('ALIOSS_CONFIG.KEY_SECRET');
            $endpoint = config('ALIOSS_CONFIG.END_POINT');
            ...
            if (file_exists($path)) {
                // 实例化oss类
                vendor('Alioss.autoload');//加载
                $oss = new \OSS\OssClient($key_id,$key_secret,$endpoint);
            ...
            $this->success('上传成功', null, [
                'url' => 'https://'.$bucket.'.'.$endpoint.'/'.$oss_path
            ]);
        }
        else
        {
    ...
    public function ajax_admin_name(){
